...
def mirror_guides(correct_side='', mirror_to=''):      
    list_correct_guides = cmds.listRelatives('%s_guides_grp'%(correct_side), ad=True)
    list_correct_guides.reverse()
    list_mirror_to_guides = cmds.listRelatives('%s_guides_grp'%(mirror_to), ad=True)
    list_mirror_to_guides.reverse()
    if len(list_correct_guides) != len(list_mirror_to_guides): cmds.warning('Choose similar modules')
